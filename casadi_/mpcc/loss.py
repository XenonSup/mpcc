...
def gen_cost_func(order):
    ...
    # investigate cd.polyval <- non-standard coefficients
    path_poly_eval = (lambda cx, cy, d: cd.vertcat(cd.polyval(cx, d), cd.polyval(cy, d)))

    s_theta = path_poly_eval(coefs_x, coefs_y, t) # pos(theta)
    r_pqs = s_theta - pos                         # pos(theta) - pos
    ...
    # outputs = [5.*e_c + 1.*e_l + .5*(t - t_dest)**2 + 1.*(a**2) + 1.*(alpha**2) + 10.*(dt**2)]   
